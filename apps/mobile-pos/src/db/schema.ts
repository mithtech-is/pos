/**
 * SQLite schema for the mobile POS.
 *
 * Identical structure to the Electron app (apps/pos-app/src/main/db/schema.sql)
 * so syncing semantics, local order numbers, and idempotency keys behave the
 * same on every client. Kept inline rather than as a .sql file because
 * expo-sqlite's API is more ergonomic with strings.
 */
...
CREATE INDEX IF NOT EXISTS idx_local_order_items_order
  ON local_order_items(local_order_id);
...
CREATE INDEX IF NOT EXISTS idx_local_sync_queue_status
  ON local_sync_queue(status);
